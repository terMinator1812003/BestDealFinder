...
def search_flipkart(product, total_products_count):
    try:
        # Format the search query and URL
        search_query = product.replace(" ", "%20")
        url = f"https://www.myntra.com/{search_query}"

        # Set user-agent headers to avoid being blocked
        headers = {
        ...
        # Create a defaultdict to store product data
        myntra_products_data = defaultdict(dict)

        # Loop over the product containers and extract relevant product data
        for i, container in enumerate(product_containers[:total_products_count]):
            # Extract product name
            name = container.find("h3", {"class": "product-brand"}).text.strip() + " " + container.find("h4", {"class": "product-product"}).text.strip()
            myntra_products_data[i]["name"] = name

            # Extract product price
            price = container.find("div", {"class": "product-price"}).text.strip()
            myntra_products_data[i]["price"] = price

            # Extract product link
            link = container.find("a", {"class": "product-base-link"})["href"]
            myntra_products_data[i]["link"] = "https://www.myntra.com" + link

        return myntra_products_data

    except Exception as e:
        print("An error occurred while fetching data from Myntra:", e)
        return {}
